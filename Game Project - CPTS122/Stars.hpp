...
void StarSpeed::loadTexture() {
    objTex.loadFromFile("textures/speed.PNG");
}

void StarSpeed::setSprite() {
    objSprite.setTexture(objTex);
    objSprite.setScale(1.0, 0.5);
}

void StarSpeed::moveStar(float speed) {
    for (auto& meteor : objects) {
        meteor.move(-speed, 0.1);
    }
